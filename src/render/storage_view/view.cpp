...
void renderStorageView(api::StorageId storageId, UserId userId, ChatId chatId, BotRef bot, StorageApiRef storageApi) {
    ...
    keyboard[0].push_back(makeCallbackButton(u8"↩️ Назад", "storage_view_back"));
    keyboard[0].push_back(makeCallbackButton(u8"👥 Участники", "storage_view_members"));
    keyboard[0].push_back(makeCallbackButton(u8"🍗 Продукты", "storage_view_explore"));
    keyboard[1].push_back(makeCallbackButton(u8"😋 Хочу кушать!", "storage_view_wanna_eat"));
