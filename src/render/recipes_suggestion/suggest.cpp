...
constructMarkup(const std::vector<api::StorageId>& storageIds, int pageNo, UserId userId, RecipesApiRef recipesApi) {
    ...
    if (pageNo == 0) {
        if (ifMaxPage) {
             ...
             */
            keyboard[recipesList.recipesPage.size()].push_back(makeCallbackButton(
                u8"↩️ Назад",
        ...
        keyboard[recipesList.recipesPage.size()].push_back(makeCallbackButton(u8"⏭️", std::to_string(pageNo + 1)));

        /* Put the number of storages.
         * If more then one then return to storage list choose if one then go to the storage view.
         * Even if one storage was chosen in storage list choose it will return to view of these one storage.
         */
        keyboard[recipesList.recipesPage.size() + 1].push_back(makeCallbackButton(
    ...
    if (messageId) {
        bot.editMessageText(pageInfo,
                            ...
                            *messageId,
                            ...
                            "",
                            nullptr,
                            makeKeyboardMarkup(constructMarkup(storageIds, pageNo, userId, recipesApi)));
    }
}
